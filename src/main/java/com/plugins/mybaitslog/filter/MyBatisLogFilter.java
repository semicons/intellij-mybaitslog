...
public class MyBatisLogFilter implements Filter {
    ...
    private void prints(final String currentLine, int endPoint) {
        ...
        final String totalStr = ConfigUtil.getTotal();
        ...
        if (!StringUtils.isEmpty(preparingLine) && currentLine.contains(totalStr)) {
            totalLine = currentLine;
        }
        if (StringUtils.isNotEmpty(preparingLine) && StringUtils.isNotEmpty(parametersLine) && StringUtils.isNotEmpty(totalLine)) {
            //序号前缀字符串
            String[] restoreSql = SqlProUtil.restoreSql(preparingLine, parametersLine,totalLine);
//            PrintlnUtil.println(project, KeyNameUtil.SQL_Line.concat(restoreSql[0]), ConsoleViewContentType.USER_INPUT);
//            PrintlnUtil.println(project, "", ConsoleViewContentType.USER_INPUT);
            ...
            for (int i = 0; i < (KeyNameUtil.SQL_Line.concat(restoreSql[2]).length()); i++) {
                splitLine.append("=");
            }
